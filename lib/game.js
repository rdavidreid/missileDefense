...
(function () {
  ...
  Game.prototype.addRound = function() {
    if (this.missiles.length === 0){
      ...
      if (this.round > 10) {
        this.ammo = (this.cities.length * 8);
        this.ammo += 30;
      }
      ...
      this.ammo = (this.cities.length * 4);
      this.ammo += 14;
      }
  ...
  Game.prototype.checkBullets = function() {
    ...
    var timeToBlowX = function(vel, currentPos,destinationPos) {
    ...
    for (var i = 0; i < that.bullets.length; i ++) {
      if (timeToBlowX(that.bullets[i].vel[0],that.bullets[i].pos[0],that.bullets[i].destination[0]) &&
  ...
  Game.prototype.draw = function(ctx) {
    ...
    ctx.fillStyle = "white";
    ctx.fillText("Ammo: " + this.ammo, 20, 110);
